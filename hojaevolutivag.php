...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-tasks"></i> Control de Hojas Evolutivas</h4>
<ol class="breadcrumb pull-right"><li><a href="panel">Inicio</a></li>
<li class="active">Hojas Evolutivas</li>
</ol>
...
<h3 class="panel-title"><i class="fa fa-tasks"></i> Control de Hojas Evolutivas<span class="pull-right"><a href="forhojaevolutivag" class="btn btn-default dropdown-toggle" data-toggle="tooltip" data-placement="left" title="" data-original-title="Nueva Hoja Evolutiva"><span class="fa fa-clipboard"></span> Nueva Hoja</a></span></h3>
</div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<center><span class='fa fa-check-square-o'></span> LA HOJA DE EVOLUCIÓN MÉDICA FUE ELIMINADA EXITOSAMENTE </center>"; 
		echo "</div>";
		break;
    ...
    echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
    echo "<center><span class='fa fa-info-circle'></span> USTED NO PUEDE ELIMINAR HOJA DE EVOLUCIÓN MÉDICA, NO ERES EL ADMINISTRADOR </center>";  
    echo "</div>"; 
    break;
...
<td><abbr title="<?php echo "Nº: ".$reg[$i]['codprocedimiento']; ?>"><?php echo $reg[$i]['procedimiento']; ?></abbr></td>
                         <td><?php echo date("d-m-Y h:i:s", strtotime($reg[$i]['fechagenerahoja'])); ?></td>
											   <td>
...
<a class="btn btn-primary btn-xs" title="Editar" onClick="actualizar('edithojaevolutivag?h=<?php echo base64_encode($reg[$i]["codprocedimiento"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<?php if ($_SESSION['acceso'] == "administrador"){ ?><a class="btn btn-danger btn-xs" onClick="eliminar('eliminar?<?php echo base64_encode('eliminar'); ?>&codprocedimiento=<?php echo base64_encode($reg[$i]["codprocedimiento"]) ?>&tipo=<?php echo base64_encode("HOJAEVOLUTIVA") ?>')" title="Eliminar" ><i class="fa fa-trash-o"></i></a><?php } ?></td>
											   </tr>
											   <?php } } ?>
											   </tbody>
</table>
