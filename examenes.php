...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-tasks"></i> Control de Solicitud Exámenes</h4>
<ol class="breadcrumb pull-right"><li><a href="panel">Inicio</a></li>
<li class="active">Solicitud de Exámenes</li>
</ol>
...
<h3 class="panel-title"><i class="fa fa-tasks"></i> Control de Solicitud Exámenes<span class="pull-right"><a href="forexamenes" class="btn btn-default dropdown-toggle" data-toggle="tooltip" data-placement="left" title="" data-original-title="Nueva Solicitud Exámenes"><span class="fa fa-list-alt"></span> Nueva Solicitud</a></span></h3>
</div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<center><span class='fa fa-check-square-o'></span> LA SOLICITUD DE EXÁMENES FUE ELIMINADA EXITOSAMENTE </center>"; 
		echo "</div>";
		break;
    ...
    echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
    echo "<center><span class='fa fa-info-circle'></span> USTED NO PUEDE ELIMINAR SOLICITUD DE EXÁMENES, NO ERES EL ADMINISTRADOR </center>";  
    echo "</div>"; 
    break;
...
<?php 
$reg = new Login();
$reg = $reg->ListarSolicitudExamenes();
...
<a href="reportepdf?se=<?php echo base64_encode($reg[$i]['codexamen']); ?>&tipo=<?php echo base64_encode("SOLICITUDEXAMEN") ?>" target="_black" rel="noopener noreferrer" class="btn btn-info btn-xs" title="Imprimir"><i class="fa fa-print"></i></a>

<a class="btn btn-primary btn-xs" title="Editar" onClick="actualizar('editexamenes?se=<?php echo base64_encode($reg[$i]["codexamen"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<?php if ($_SESSION['acceso'] == "administrador"){ ?><a class="btn btn-danger btn-xs" onClick="eliminar('eliminar?<?php echo base64_encode('eliminar'); ?>&codexamen=<?php echo base64_encode($reg[$i]["codexamen"]) ?>&tipo=<?php echo base64_encode("SOLICITUDEXAMEN") ?>')" title="Eliminar" ><i class="fa fa-trash-o"></i></a><?php } ?></td>
											   </tr>
											   <?php } } ?>
											   </tbody>
</table>
