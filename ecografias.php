...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-tasks"></i> Control de Ecografías</h4>
<ol class="breadcrumb pull-right"><li><a href="panel">Inicio</a></li>
<li class="active">Ecografías</li>
</ol>
...
<h3 class="panel-title"><i class="fa fa-tasks"></i> Control de Ecografías<span class="pull-right"><a href="forecografia" class="btn btn-default dropdown-toggle" data-toggle="tooltip" data-placement="left" title="" data-original-title="Nueva Ecografía"><span class="fa fa-clipboard"></span> Nueva Ecografía</a></span></h3>
</div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<center><span class='fa fa-check-square-o'></span> LA ECOGRAFIA FUE ELIMINADA EXITOSAMENTE </center>"; 
		echo "</div>";
		break;
    ...
    echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
    echo "<center><span class='fa fa-info-circle'></span> USTED NO PUEDE ELIMINAR ECOGRAFIAS, NO ERES EL ADMINISTRADOR </center>";  
    echo "</div>"; 
    break;
...
<td><abbr title="<?php echo "Nº de Identificación : ".$reg[$i]['cedpaciente']; ?>"><?php echo $reg[$i]['pnompaciente']." ".$reg[$i]['snompaciente']." ".$reg[$i]['papepaciente']." ".$reg[$i]['sapepaciente']; ?></abbr></td>
                         <td><?php echo date("d-m-Y h:i:s", strtotime($reg[$i]['fechaecografia'])); ?></td>
											   <td>
<a href="reportepdf?ec=<?php echo base64_encode($reg[$i]['codecografia']); ?>&tipo=<?php echo base64_encode("ECOGRAFIAS") ?>" target="_black" rel="noopener noreferrer" class="btn btn-info btn-xs" title="Imprimir"><i class="fa fa-print"></i></a>

<a class="btn btn-primary btn-xs" title="Editar" onClick="actualizar('editecografia?ec=<?php echo base64_encode($reg[$i]["codecografia"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<?php if ($_SESSION['acceso'] == "administrador"){ ?><a class="btn btn-danger btn-xs" onClick="eliminar('eliminar?<?php echo base64_encode('eliminar'); ?>&codecografia=<?php echo base64_encode($reg[$i]["codecografia"]) ?>&tipo=<?php echo base64_encode("ECOGRAFIAS") ?>')" title="Eliminar" ><i class="fa fa-trash-o"></i></a><?php } ?></td>
											   </tr>
											   <?php } } ?>
											   </tbody>
</table>
