...
{
$reg = $tra->RegistrarPlantillaEcografia();
exit;
} 
else if(isset($_POST['btn-update']))
{
$reg = $tra->ActualizarPlantillaEcografia();
exit;
}
?><!DOCTYPE html>
<html>
...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-tasks"></i> Gestión de Plantillas Ecográficas</h4>
<ol class="breadcrumb pull-right"><li><a href="panel">Inicio</a></li>
<li><a href="plantillasginecologia">Control</a></li>
...
<?php  if (isset($_GET['codplantillaecografia'])) {
      
      $reg = $tra->PlantillaEcografiaPorId(); ?>
      
<form class="form" name="updateplantillaecografia" id="updateplantillaecografia" method="post" data-id="<?php echo $reg[0]["codplantillaecografia"] ?>" action="#">
        
    <?php } else { ?>
        
<form class="form" method="post"  action="#" name="plantillaecografia" id="plantillaecografia"> 
...
<div class="panel-heading"><h3 class="panel-title"><i class="fa fa-search"></i> Médicos Ginecologos</h3></div>
...
<?php 
$tro = new Login();
$rog = $tro->ListarMedicosEspecialidadGinecologia();
...
<div class="col-sm-12">
<div class="panel panel-primary">
<div class="panel-heading"><h3 class="panel-title"><i class="fa fa-edit"></i> Gestión de Plantillas Ecográficas
</h3></div>
<div class="panel-body">
<div class="row">
<div class="col-sm-12 col-xs-12">
...
<input type="hidden" name="codplantillaecografia" id="codplantillaecografia" <?php if (isset($reg[0]['codplantillaecografia'])) { ?> value="<?php echo $reg[0]['codplantillaecografia']; ?>"<?php } ?>>

        
         <?php if (isset($reg[0]['idplantillaecografia'])) { ?>
  ...
  <label class="control-label">Procedimientos: <span class="symbol required"></span></label> 
    <input type="text" class="form-control" name="procedimientoecografia" id="procedimientoecografia" onKeyUp="this.value=this.value.toUpperCase();" autocomplete="off" placeholder="Ingrese Procedimientos" <?php if (isset($reg[0]['procedimientoecografia'])) { ?> value="<?php echo $reg[0]['procedimientoecografia']; ?>"<?php } ?> required="" aria-required="true">
                        <i class="fa fa-pencil form-control-feedback"></i>  
                              </div> 
                        </div>
                    </div>
 ...
 <div class="col-md-12"> 
                               <div class="form-group has-feedback"> 
  <label class="control-label">Descripción Plantilla Ecográfica: <span class="symbol required"></span></label> 
 <textarea name="descripcionecografia" class="form-control" id="descripcionecografia" onKeyUp="this.value=this.value.toUpperCase();" autocomplete="off" placeholder="Ingrese Descripción de Plantilla Ecográfica" rows="10" required="" aria-required="true"><?php if (isset($reg[0]['descripcionecografia'])) { ?><?php echo htmlspecialchars($reg[0]['descripcionecografia']); ?><?php } ?></textarea>
                        <i class="fa fa-pencil form-control-feedback"></i>  
                              </div> 
                        </div>  
             </div><br>
