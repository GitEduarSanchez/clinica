...
<button type="button" class="close" data-dismiss="modal" aria-hidden="true"><img src="assets/images/close.png"/></button> 
             <h3 class="panel-title"><i class="fa fa-align-justify"></i> Datos de Apertura</h3> 
                                                        </div> 
                                                        <div class="panel-body"> 
                                                         <div id="muestraaperturamodal"></div>
                                                        </div>
...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-tasks"></i> Control de Aperturas de Historias</h4>
<ol class="breadcrumb pull-right"><li><a href="panel">Inicio</a></li>
<li class="active">Aperturas de Historias</li>
</ol>
...
<div class="panel-heading">
<h3 class="panel-title"><i class="fa fa-tasks"></i> Control de Aperturas de Historias<span class="pull-right"><a href="forapertura" class="btn btn-default dropdown-toggle" data-toggle="tooltip" data-placement="left" title="" data-original-title="Nueva Apertura de Historia"><span class="fa fa-folder-open"></span> Nueva Apertura</a></span></h3>
</div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<center><span class='fa fa-check-square-o'></span> LA APERTURA DE HISTORIA MÉDICA FUE ELIMINADA EXITOSAMENTE </center>"; 
		echo "</div>";
		break;
    ...
    echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
    echo "<center><span class='fa fa-info-circle'></span> USTED NO PUEDE ELIMINAR APERTURA DE HISTORIA MÉDICA, NO ERES EL ADMINISTRADOR </center>";  
    echo "</div>"; 
    break;
...
<td><abbr title="<?php echo "Nº de Identificación : ".$reg[$i]['cedpaciente']; ?>"><?php echo $reg[$i]['pnompaciente']." ".$reg[$i]['snompaciente']." ".$reg[$i]['papepaciente']." ".$reg[$i]['sapepaciente']; ?></abbr></td>
                         <td><?php echo date("d-m-Y h:i:s", strtotime($reg[$i]['fechaapertura'])); ?></td>
											   <td>
<!--<a class="btn btn-success btn-xs" data-placement="left" title="Ver" data-original-title="" data-href="#" data-toggle="modal" data-target="#panel-modal" data-backdrop="static" data-keyboard="false" onClick="VerApertura('<?php echo base64_encode($reg[$i]["codapertura"]); ?>')"><i class="fa fa-eye"></i></a>-->
...
<a class="btn btn-primary btn-xs" title="Editar" onClick="actualizar('editapertura?a=<?php echo base64_encode($reg[$i]["codapertura"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<?php if ($_SESSION['acceso'] == "administrador"){ ?><a class="btn btn-danger btn-xs" onClick="eliminar('eliminar?<?php echo base64_encode('eliminar'); ?>&codapertura=<?php echo base64_encode($reg[$i]["codapertura"]) ?>&tipo=<?php echo base64_encode("APERTURA") ?>')" title="Eliminar" ><i class="fa fa-trash-o"></i></a><?php } ?></td>
											   </tr>
											   <?php } } ?>
											   </tbody>
</table>
