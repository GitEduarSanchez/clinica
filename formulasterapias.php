...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-tasks"></i> Control de Fórmulas Terapias</h4>
<ol class="breadcrumb pull-right"><li><a href="panel">Inicio</a></li>
<li class="active">Fórmulas Terapias</li>
</ol>
...
<h3 class="panel-title"><i class="fa fa-tasks"></i> Control de Fórmulas Terapias<span class="pull-right"><a href="forfterapia" class="btn btn-default dropdown-toggle" data-toggle="tooltip" data-placement="left" title="" data-original-title="Nueva Fórmula"><span class="fa fa-file-text"></span> Nueva Fórmula</a></span></h3>
</div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<center><span class='fa fa-check-square-o'></span> LA FÓRMULA DE TERAPIA FUE ELIMINADA EXITOSAMENTE </center>"; 
		echo "</div>";
		break;
    ...
    echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
    echo "<center><span class='fa fa-info-circle'></span> USTED NO PUEDE ELIMINAR FÓRMULAS DE TERAPIAS, NO ERES EL ADMINISTRADOR </center>";  
    echo "</div>"; 
    break;
...
<?php 
$reg = new Login();
$reg = $reg->ListarFormulasTerapias();
...
<td><abbr title="<?php echo "Nº de Identificación : ".$reg[$i]['cedpaciente']; ?>"><?php echo $reg[$i]['pnompaciente']." ".$reg[$i]['snompaciente']." ".$reg[$i]['papepaciente']." ".$reg[$i]['sapepaciente']; ?></abbr></td>
                         <td><?php echo date("d-m-Y h:i:s", strtotime($reg[$i]['fechaformulaterapia'])); ?></td>
											   <td>
...
<a class="btn btn-primary btn-xs" title="Editar" onClick="actualizar('editfterapia?ft=<?php echo base64_encode($reg[$i]["codfterapia"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<?php if ($_SESSION['acceso'] == "administrador"){ ?><a class="btn btn-danger btn-xs" onClick="eliminar('eliminar?<?php echo base64_encode('eliminar'); ?>&codfterapia=<?php echo base64_encode($reg[$i]["codfterapia"]) ?>&tipo=<?php echo base64_encode("FORMULASTERAPIAS") ?>')" title="Eliminar" ><i class="fa fa-trash-o"></i></a><?php } ?></td>
											   </tr>
											   <?php } } ?>
											   </tbody>
</table>
