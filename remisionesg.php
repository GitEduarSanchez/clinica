...
<div class="row">
<div class="col-sm-12">
<div class="page-header-title"><h4 class="pull-left page-title"><i class="fa fa-tasks"></i> Control de Remisiones</h4>
...
<h3 class="panel-title"><i class="fa fa-tasks"></i> Control de Remisiones<span class="pull-right"><a href="forremisiong" class="btn btn-default dropdown-toggle" data-toggle="tooltip" data-placement="left" title="" data-original-title="Nueva Remisión"><span class="fa fa-file-text"></span> Nueva Remisión</a></span></h3>
</div>
		...
		if(isset($_GET["mesage"]))
{
	switch($_GET["mesage"])
	{
		...
		echo "<center><span class='fa fa-check-square-o'></span> LA REMISIÓN FUE ELIMINADA EXITOSAMENTE </center>"; 
		echo "</div>";
		break;
    ...
    echo "<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>";
    echo "<center><span class='fa fa-info-circle'></span> USTED NO PUEDE ELIMINAR REMISIONES, NO ERES EL ADMINISTRADOR </center>";  
    echo "</div>"; 
    break;
...
<a class="btn btn-primary btn-xs" title="Editar" onClick="actualizar('editremisiong?r=<?php echo base64_encode($reg[$i]["codprocedimiento"]) ?>')"><i class="fa fa-pencil"></i></a>
                                 
<?php if ($_SESSION['acceso'] == "administrador"){ ?><a class="btn btn-danger btn-xs" onClick="eliminar('eliminar?<?php echo base64_encode('eliminar'); ?>&codremision=<?php echo base64_encode($reg[$i]["codremision"]) ?>&tipo=<?php echo base64_encode("REMISIONES") ?>')" title="Eliminar" ><i class="fa fa-trash-o"></i></a><?php } ?></td>
											   </tr>
											   <?php } } ?>
											   </tbody>
</table>
